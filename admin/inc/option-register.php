...
function mishie_textfield( $options, $option_name, $option_label = '', $option_type = 'text', $option_class = 'regular-text', $label_after = '' ) {
?>
	<p><?php
	if ( ! empty( $option_label ) ) {
		echo '<label for="mishie_theme_options[' . esc_attr( $option_name ) . ']">' . esc_attr( $option_label ) . '</label>';
	} ?>
	<input id="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" class="<?php echo esc_attr( $option_class ); ?>" type="<?php echo esc_attr( $option_type ); ?>" name="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" value="<?php
	switch ( $option_type ){
		case 'url':
...
function mishie_textarea( $options, $option_name, $option_cols = '60', $option_rows = '3', $content = '' ) {
	$content =  ( ! empty( $content ) ) ? $content : $options[$option_name];
?>
	<p><textarea id="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" cols="<?php echo absint( $option_cols ); ?>" rows="<?php echo absint( $option_rows ); ?>" name="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]"><?php echo esc_textarea( $content ); ?></textarea></p>
<?php
}
...
function mishie_checkbox( $options, $option_name, $option_text = '', $text_desc = '' ) {
?>
	<p class="checkbox"><label><input id="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" name="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" type="checkbox" value="1" <?php checked( $options[$option_name], 1 ); ?> />
...
function mishie_radio( $options, $option_array, $option_name ) {
...
function mishie_radio_input( $options, $option, $option_name ) {
	$option_label = ( ! empty( $option['label'] ) ) ? '&nbsp;' . $option['label'] : '';
	$option_class = ( ! empty( $option['label'] ) ) ? 'right-space' : 'no-space';
?>
	<label class="<?php echo esc_attr( $option_class ); ?>"><input type="radio" name="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" value="<?php echo esc_attr( $option['value'] ); ?>" <?php checked( $options[$option_name], $option['value'] ); ?> /><?php echo esc_attr( $option_label ); ?>
...
function mishie_select( $options, $option_array, $option_name, $option_label = '', $option_label_before = '' ) {
	echo '<p>';

	if ( ! empty( $option_label ) ) : ?>
		<label for="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]"><?php echo esc_attr( $option_label ); ?></label>
	<?php
		echo ( $option_label_before == 'br' ) ? '<br />' : esc_attr( $option_label_before ) ;
	endif;
?>
	<select id="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" name="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" >
...
function mishie_color_picker( $options, $option_name, $default_color ) {
	$default_color = mishie_sanitize_hex_color( $default_color );

?>
	<div class="color-picker">
		<input id="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" name="mishie_theme_options[<?php echo esc_attr( $option_name ); ?>]" value="<?php
	$color = mishie_sanitize_hex_color( $options[$option_name] );
...
function mishie_sanitize_hex_color( $color ) {
	if ( '' === $color )
		return '';
...
function mishie_media_uploader( $options, $option_id, $option_name, $option_desc = '' ) {
